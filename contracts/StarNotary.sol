...
contract StarNotary is ERC721 {
    ...
    string public name = "GaTestTokenExample";
    ...
    function createStar(string memory _name, uint256 _tokenId) public { // Passing the name and tokenId as a parameters
        Star memory newStar = Star(_name, 'GAT'); // Star is an struct so we are creating a new Star
    ...
    function strConcat(string memory _a, string memory _b, string memory _c) internal pure returns (string memory){
        ...
        bytes memory bac = bytes(abc);
        uint k = 0;
        uint i = 0;
        for (i = 0; i < _ba.length; i++) bac[k++] = _ba[i];
        for (i = 0; i < _bb.length; i++) bac[k++] = _bb[i];
        for (i = 0; i < _bc.length; i++) bac[k++] = _bc[i];
        return string(bac);
    }
    ...
    function lookUptokenIdToStarInfo (uint _tokenId) public view returns (string memory) {
        ...
        return strConcat(tokenIdToStarInfo[_tokenId].name, '-', tokenIdToStarInfo[_tokenId].symbol);
    }
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        if (ownerOf(_tokenId1) != msg.sender && ownerOf(_tokenId2) != msg.sender)
            revert("You can't transfer Stars without a valid owner");
